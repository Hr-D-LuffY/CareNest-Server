...
const assertRatesMatchType = (
  staffType: StaffType,
  { hourlyRate, perMinuteRate }: Rates,
) => {
...
const updateStaff = async (staffId: string, payload: UpdateStaffPayload) => {
  ...
  const { name, staffType, bio, experience, hourlyRate, perMinuteRate } =
    payload;
  ...
  assertRatesMatchType(staffType ?? current.staffType, {
    ...
    perMinuteRate:
      perMinuteRate === undefined ? current.perMinuteRate : perMinuteRate,
  });
...
const verifyStaff = async (
  ...
  if (current.verificationStatus === status) {
    throw new AppError(
      httpStatus.CONFLICT,
      `Staff is already ${status.toLowerCase()}`,
    );
  }
...
const getTopRatedStaff = async () => {
  ...
  const nameById = new Map(
    staff.map((member) => [member.id, member.user.name]),
  );
...
const getDashboardStats = async () => {
  ...
  return {
    ...
    waitlistConversionRate:
      totalWaitlisted === 0 ? 0 : promotedWaitlisted / totalWaitlisted,
    topRatedStaff,
  };
};
